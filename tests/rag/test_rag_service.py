...
import pytest
import re
from src.rag.models import Chunk, Paragraph, SearchResult
from src.rag.evidence_policy import EvidencePolicy
from src.rag.generator import INSUFFICIENT_EVIDENCE
...
def test_build_index_saves_all_chunks(
    ...
    monkeypatch.setattr(
        "src.rag.service.load_directory",
        lambda _: (
            {
                "guide.docx": [
...
def test_service_logs_retrieval_and_generation(caplog) -> None:
    ...
    with caplog.at_level("INFO", logger="src.rag.service"):
        service.ask("How do I declare parameters?")
...
def test_service_logs_retrieval_failure(caplog) -> None:
    ...
    with caplog.at_level("ERROR", logger="src.rag.service"):
